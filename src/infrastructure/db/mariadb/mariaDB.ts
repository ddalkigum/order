...
import OrderEntity from './entity/order/orders';
import CategoryEntity from './entity/restaurant/categories';
import RestaurantEntity from './entity/restaurant/restaurants';
import UserEntity from './entity/user/users';
...
export default class MariaDB implements IDatabase {
  ...
  public async getDataListByColumn<T extends IEntity>(table: string, columnCondition: ColumnCondition<T>): Promise<T[]> {
    try {
      const EntityClass = getEntityByTableName(table);
      const objectLiteral = convertGetDataColumnCondition(columnCondition);
      this.logger.debug(`objectLiteral: ${JSON.stringify(objectLiteral)}`);
      const result = await this.connection.createQueryBuilder<T>(EntityClass, table).where(objectLiteral).getMany();
      return result;
    } finally {
      // await this.close();
    }
  ...
  public async getAllData<T extends IEntity>(table: string) {
    const EntityClass = getEntityByTableName(table);
  ...
  public async deleteDataByColumn<T extends IEntity>(table: string, columnCondition: ColumnCondition<T>): Promise<void> {
    const queryRunner = this.connection.createQueryRunner();
    try {
      const EntityClass = getEntityByTableName(table);
      await queryRunner.startTransaction();
      const objectLiteral = convertGetDataColumnCondition(columnCondition);
      await this.connection.createQueryBuilder<T>(EntityClass, table).delete().from(EntityClass).where(objectLiteral).execute();
      await queryRunner.commitTransaction();
    } catch (error) {
      await queryRunner.rollbackTransaction();
      throw error;
